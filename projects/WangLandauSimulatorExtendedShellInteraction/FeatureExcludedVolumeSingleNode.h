#ifndef FEATURE_EXCLUDED_VOLUME_H
#define FEATURE_EXCLUDED_VOLUME_H
...
/**
 * @brief Forward declaration. Equivalent to FeatureExcludedVolumeSingleNode< FeatureLattice <bool> >
 */
template<class SpecializedClass = FeatureLattice<> > class FeatureExcludedVolumeSingleNode;
 ...
 * lattice, one could use FeatureExcludedVolumeSingleNode< FeatureLattice <int8_t> > or
 * for specialized (more performance) a lattice of power 2 type: FeatureExcludedVolumeSingleNode< FeatureLatticePowerOfTwo <int8_t> >.
...
class FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> > : public Feature {
public:
	//! This Feature requires a lattice.
	typedef LOKI_TYPELIST_1(SpecializedClass<ValueType>) required_features_front;

	//constructor
	FeatureExcludedVolumeSingleNode() :
...
template<template<typename> class SpecializedClass, typename ValueType>
template<class IngredientsType>
bool FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::checkMove(
...
bool FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::checkMove(
	...
	if (!latticeFilledUp)
		throw std::runtime_error(
				"*****FeatureExcludedVolumeSingleNode_T::checkMove....lattice is not populated. Run synchronize!\n");
...
#ifdef DEBUG
	if(!latticeFilledUp)
	throw std::runtime_error("*****FeatureExcludedVolumeSingleNode_T::checkMove....lattice is not populated. Run synchronize!\n");
#endif
...
template<template<typename> class SpecializedClass, typename ValueType>
template<class IngredientsType>
void FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::applyMove(IngredientsType& ing,
		const MoveBase& move)
		{
...
void FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::applyMove(IngredientsType& ing,
	...
	if (!latticeFilledUp)
		throw std::runtime_error(
				"*****FeatureExcludedVolumeSingleNode_T::applyMove....lattice is not populated. Run synchronize!\n");
...
template<template<typename> class SpecializedClass, typename ValueType>
template<class IngredientsType>
void FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::synchronize(
		IngredientsType& ingredients)
		{
	//note: the lattice entries are set to 0 before by the 
	//synchronize function of FeatureLattice
	std::cout << "FeatureExcludedVolumeSingleNode_T::synchronizing lattice occupation...";
...
template<template<typename> class SpecializedClass, typename ValueType>
template<class IngredientsType>
void FeatureExcludedVolumeSingleNode< SpecializedClass<ValueType> >::fillLattice(
		IngredientsType& ingredients)
		{
	...
	for (size_t n = 0; n < molecules.size(); n++)
	{
		...
		{		      
			...
			if (ingredients.getLatticeEntry(pos))
			{
				...
				errorMessage << "FeatureExcludedVolumeSingleNode<>::fillLattice(): lattice already occupied when trying to write monomer ";
