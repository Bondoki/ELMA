...
#include "FeatureExtendedShellInteraction.h"
#include "UpdaterAdaptiveWangLandauSamplingExtendedShellInteraction.h"
...
int main(int argc, char* argv[])
{
	try{
		...
		else if(showHelp == true)
		{
			std::cout << "Simulator for the ScBFM with Ex.Vol and BondCheck and WangLandau in NN-shell" << std::endl
					...
					<< "Features used: FeatureBondset, FeatureAttributes, FeatureExcludedVolumeSc<FeatureLattice<uint8_t> >, FeatureWangLandauExtendedShellInteraction" << std::endl
	...
	typedef LOKI_TYPELIST_3(FeatureMoleculesIO, FeatureAttributes, FeatureExtendedShellInteraction< FeatureLattice >) Features;
...
#pragma omp parallel private(nthreads, tid) shared(mol, energyState, energyWinStart, energyWinEnd, lnDOSenergyOld, lnDOSenergyNew, rnd_tid, acceptExchange, counterCovergedIteration)
	{
		...
		*/
		...
		UpdaterAdaptiveWangLandauSamplingExtendedShellInteraction<Ing,MoveLocalSc> UWL(myIngredients,
						save_interval,
...
#pragma omp barrier
		...
		taskmanager.addUpdater(new UpdaterAdaptiveWangLandauSamplingExtendedShellInteraction<Ing,MoveLocalSc>(myIngredients,
				save_interval,
